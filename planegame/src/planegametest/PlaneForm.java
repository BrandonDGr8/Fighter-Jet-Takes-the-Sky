...
public class PlaneForm implements MouseListener
{
	...
	public final int X1POS;
	public final int X2POS;
	...
	private MovingImage plane1;
	private MovingImage plane2;
	...
	public void initiate()
	{
		...
		plane1 = new MovingImage("StraightPlane1.png",X1POS,400);
		plane2 = new MovingImage("StraightPlane2.png",X2POS,414);
		...
		middlerecs.add(new MovingImage("Missile.png",1682,randomMidHeight()));
...
//				}
			...
			back.updateImages(toprecs,middlerecs,bottomrecs,plane1,plane2,smoke);
		}
	...
	public void updateMiddle(double a)
	{
		...
		if(middlerecs.get(0).getX() > -1 * RECWIDTH)
		{
			...
			middlerecs.set(0,new MovingImage("Missile.png",middlerecs.get(0).getX() - (RECWIDTH/div), middlerecs.get(0).getY()));
			middlerecs.set(1,new MovingImage("Missile.png",middlerecs.get(1).getX() - (RECWIDTH/div), middlerecs.get(1).getY()));
			middlerecs.set(2,new MovingImage("Missile.png",middlerecs.get(2).getX() - (RECWIDTH/div), middlerecs.get(2).getY()));
		...
		{
			middlerecs.set(0,new MovingImage("Missile.png",middlerecs.get(1).getX() - (RECWIDTH/div), middlerecs.get(1).getY()));
			middlerecs.set(1,new MovingImage("Missile.png",middlerecs.get(2).getX() - (RECWIDTH/div), middlerecs.get(2).getY()));
			middlerecs.set(2,new MovingImage("Missile.png",middlerecs.get(0).getX() + 580,randomMidHeight()));
	...
	{
		for(int x = 3; x <= 7; x++)
			if(plane1.getY() + 35 >= bottomrecs.get(x).getY())
				return true;

		for(int y = 3; y <= 7; y++)
			if(plane1.getY() <= toprecs.get(y).getY() + RECHEIGHT)
				return true;
		for(int z = 0; z <= 1; z++)
			if(isInMidRange(z))
				return true;
	...
	public boolean isInMidRange(int num)
	{
		...
		Rectangle planecheck = new Rectangle((int)(plane1.getX()+plane2.getx()),(int)(plane1.getY()+plane2.getY()),106,35);
	...
	public void updatePlane()
	{
		...
		{
			if(upCount < 3.5) {
				plane1.setPosition(XPOS,(double)(plane.getY() - (.3 + upCount)));
				plane2.setPosition(XPOS,(double)(plane.getY() - (.3 + upCount)));
			}
			else {
				plane1.setPosition(XPOS,(double)(plane.getY() - (1.2 + upCount)));
				plane2.setPosition(XPOS,(double)(plane.getY() - (1.2 + upCount)));
			}
			plane1.setImage("UpPlane.png");	
			plane2.setImage();
		}
		else
		{
			if(upCount < 1) {
				plane1.setPosition(XPOS,(double)(plane1.getY() + upCount));
				plane2.setPosition(XPOS,(double)(plane2.getY() + upCount));
			}
			else {
				plane1.setPosition(XPOS,(double)(plane1.getY() + (1.2 + upCount)));
				plane2.setPosition(XPOS,(double)(plane2.getY() + (1.2 + upCount)));
			}
			plane1.setImage("StraightPlane1.png");
			plane2.setImage("StraightPlane2.png");
		}
		if(shoot())
			crash();
	}
...
class ImagePanel extends JPanel {
	...
	public void paintComponent(Graphics g) 
	{
		...
		for(MovingImage img : smoke)
			g.drawImage(img.getImage(), (int)(img.getX()), (int)(img.getY()), null);
		if(plane1 != null)
			g.drawImage(plane.getImage(), (int)(plane1.getX()), (int)(plane1.getY()), null);
		if(plane2 != null)
			g.drawImage(plane.getImage(), (int)(plane2.getX()), (int)(plane2.getY()), null);
		drawStrings(g);
	}
	...
	public void updateImages(ArrayList<MovingImage> newTop,ArrayList<MovingImage> newMiddle,ArrayList<MovingImage> newBottom,MovingImage newPlane1,newPlane2,ArrayList<MovingImage> newSmoke)
	{
		top = newTop;
		plane1 = newPlane1;
		plane2 = newPlane2;
